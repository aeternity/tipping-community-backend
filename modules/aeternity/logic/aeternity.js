...
const aeternity = {
    ...
    async claimTipsOnContract(address, url, trace, contract) {
        try {
            ...
            if (e.message && e.message.includes('NO_ZERO_AMOUNT_PAYOUT'))
                return null; // ignoring this
            if (e.message && e.message.includes('URL_NOT_EXISTING'))
                throw new Error(`Could not find any tips for url ${url}`);
            if (e.message && e.message.includes('404: Account not found'))
                return logger.info(`User ${address} has balance 0`);
            throw new Error(e);
        }
    },
