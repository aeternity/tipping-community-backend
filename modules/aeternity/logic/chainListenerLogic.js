...
const subscribeToContract = contract => {
    if (!wsconnection)
        throw new Error('init ws connection first');
...
const handleConnectionInit = async connection => {
    ...
    if (process.env.CONTRACT_V1_ADDRESS)
        subscribeToContract(process.env.CONTRACT_V1_ADDRESS);
    if (process.env.CONTRACT_V2_ADDRESS)
        subscribeToContract(process.env.CONTRACT_V2_ADDRESS);
    if (process.env.CONTRACT_V3_ADDRESS)
        subscribeToContract(process.env.CONTRACT_V3_ADDRESS);
    if (process.env.WORD_REGISTRY_CONTRACT)
        subscribeToContract(process.env.WORD_REGISTRY_CONTRACT);
