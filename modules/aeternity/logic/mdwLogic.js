...
const { ChainName, sequelize } = models;
...
const MdwLogic = {
  ...
  async middlewareContractTransactions(upperHeight, lowerHeight) {
    try {
      ...
      return result.reduce((acc, tx) => [...acc, ...aeternity.decodeTransactionEvents(tx)], []);
    } catch (e) {
  ...
  async updateChainNamesDB() {
    await lock.acquire("MdwLogic.updateChainNamesDB", async () => {
      const result = await MdwLogic.getChainNames().then((res) => Object.entries(res).map(([publicKey, chainNames]) => ({ publicKey, name: chainNames[0] })));
