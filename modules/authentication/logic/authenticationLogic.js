    ...
    async signatureAuth(req, res, next) {
        ...
        MemoryQueue.forEach(({ timestamp, file }, index) => {
            if (timestamp < Date.now() - 5 * 60 * 1000) {
                ...
                try {
                    if (file)
                        imageLogic.deleteImage(file.filename);
                }
                catch (e) {
        ...
        if (signature || challenge) {
            try {
                if (!signature)
                    return sendError('Missing field signature');
                if (!challenge)
                    return sendError('Missing field challenge');
                // Find item
                // MemoryQueue probably has a significant list deleted items
                const queueItem = MemoryQueue.find(item => (item || {}).challenge === challenge);
                if (!queueItem)
                    return sendError('Could not find challenge (maybe it already expired?)');
                ...
                if (req.method !== method)
                    return sendError('Challenge was issued for a different http method');
                if (urlParser.parse(req.originalUrl).pathname !== url)
                    return sendError('Challenge was issued for a different path');
                ...
                if (!publicKey)
                    sendError('Could not find associated public key');
            ...
            if (!req.body.author && !req.params.author)
                return sendError('Missing field author in body or url');
            const uuid = uuidv4();
            try {
                const action = actions
                    .find(({ method, path: currentPath }) => method === req.method && req.originalUrl.match(currentPath));
                if (!action)
                    return sendError('Could not find valid action related to this request');
