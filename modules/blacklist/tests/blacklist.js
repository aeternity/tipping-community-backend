...
describe("Blacklist", () => {
  ...
  describe("Blacklist API", () => {
    ...
    it("it should REJECT a new blacklist entry via admin auth for unkown id", async () => {
      const res = await chai.request(server).post("/blacklist/api").auth(process.env.AUTHENTICATION_USER, process.env.AUTHENTICATION_PASSWORD).send({
        tipId,
      });
  ...
  describe("Blacklist Frontend", () => {
    ...
    it("it should be possible to search the blacklist ui", async () => {
      ...
      const res = await chai.request(server).get("/blacklist?search=test").auth(process.env.AUTHENTICATION_USER, process.env.AUTHENTICATION_PASSWORD);
    ...
    it("it should be possible to search the blacklist ui with special chars (+)", async () => {
      ...
      const res = await chai.request(server).get("/blacklist?search=test+title2").auth(process.env.AUTHENTICATION_USER, process.env.AUTHENTICATION_PASSWORD);
      res.text.should.contain("<p>test title2</p>");
      res.should.have.status(200);
    });
    it("it should show tips only by a certain address", async () => {
      ...
      const res = await chai.request(server).get(`/blacklist?address=${sender1}`).auth(process.env.AUTHENTICATION_USER, process.env.AUTHENTICATION_PASSWORD);
      res.should.have.status(200);
    ...
    it("it should show tip by id", async () => {
      ...
      const res = await chai.request(server).get("/blacklist?id=9999_v1").auth(process.env.AUTHENTICATION_USER, process.env.AUTHENTICATION_PASSWORD);
      res.should.have.status(200);
    ...
    it("it should filter only tips", async () => {
      ...
      const res = await chai.request(server).get("/blacklist?type=tips").auth(process.env.AUTHENTICATION_USER, process.env.AUTHENTICATION_PASSWORD);
    ...
    it("it should navigate to second page", async () => {
      ...
      await seedDB({ tips }, true);
      const res = await chai.request(server).get("/blacklist?page=2").auth(process.env.AUTHENTICATION_USER, process.env.AUTHENTICATION_PASSWORD);
