...
const CacheLogic = {
  ...
  keepHot() {
    const keepHotLogic = async () =>
      lock.acquire("keepHotLogic", async () => {
  ...
  async fetchPrice() {
    return cache.getOrSet(
      ...
      async () =>
        axios
          .get("https://api.coingecko.com/api/v3/simple/price?ids=aeternity&vs_currencies=usd,eur,cny")
          .then((res) => res.data)
          .catch(logger.error),
      cache.longCacheTime,
    );
  ...
  async triggerGetTokenContractIndex(tips) {
    return lock.acquire("CacheLogic.triggerTokenContractIndex", async () => {
      ...
      const tokenRegistryContracts = await CacheLogic.getTokenRegistryState().then((state) => state.map(([token]) => token));
      // can be optimized at a later point in time with general refactoring
      return [...new Set(tokenContracts.concat(tokenRegistryContracts))].reduce(async (promiseAcc, address) => {
  ...
  async wordSaleVotesDetails(address) {
    const votes = await cache.getOrSet(["wordSaleVotes", address], () => aeternity.wordSaleVotes(address), cache.shortCacheTime);
    return Promise.all(votes.map(([id, vote]) => CacheLogic.wordSaleVoteInfo(id, vote[1], vote[0], address)));
  },
  getWordSaleTokenAddress(sale) {
    return cache.getOrSet(["wordSaleTokenAddress", sale], () => aeternity.wordSaleTokenAddress(sale));
  },
  async wordSaleVoteInfo(id, vote, alreadyApplied, sale) {
    const state = cache.getOrSet(["wordSaleVoteState", vote], () => aeternity.wordSaleVoteState(vote), cache.shortCacheTime);
    const voteTimeout = cache.getOrSet(["wordSaleVoteTimeout", sale], () => aeternity.wordSaleVoteTimeout(sale));
    const height = await aeternity.getHeight();
