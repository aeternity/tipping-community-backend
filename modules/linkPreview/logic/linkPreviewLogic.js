...
const metascraper = metascraper$0([metascraperDescription(), metascraperImage(), metascraperLang(), metascraperTitle(), metascraperUrl()]);
...
const LinkPreviewLogic = {
  ...
  async fetchAllUrls() {
    return LinkPreview.aggregate("requestUrl", "DISTINCT", { plain: false }).then((results) => results.map((preview) => preview.DISTINCT));
  },
