...
describe("Pinning", () => {
  ...
  describe("Pinning API", () => {
    ...
    it("it should reject CREATING a new pin with invalid type", (done) => {
      performSignedJSONRequest(server, "post", `/pin/${publicKey}`, { ...testData, type: "PIN" }).then(({ res }) => {
        res.should.have.status(400);
        done();
      });
    });
    it("it should CREATE a new pin via signature auth", (done) => {
      performSignedJSONRequest(server, "post", `/pin/${publicKey}`, testData).then(({ res, signature, challenge }) => {
        res.should.have.status(200);
    ...
    it("it should REMOVE a pinned entry", (done) => {
      performSignedJSONRequest(server, "delete", `/pin/${publicKey}`, testData).then(({ res }) => {
        res.should.have.status(200);
        done();
      });
    });
