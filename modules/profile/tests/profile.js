...
describe("Profile", () => {
  ...
  describe("Profile API", () => {
    ...
    it("it should CREATE a new profile", (done) => {
      const stub = jest.spyOn(ae, "getAddressForChainName").mockClear().mockImplementation(() => ({
        pointers: [{ id: publicKey, key: "account_pubkey" }],
      }));
      performSignedJSONRequest(server, "post", `/profile/${publicKey}`, testData).then(({ res, signature, challenge }) => {
        res.should.have.status(200);
        res.body.should.be.a("object");
  ...
  describe("Profile Image API", () => {
    ...
    it("it should GET an profile image", (done) => {
      chai
        ...
        .end((err, res) => {
          ...
          expect(hash(res.body).toString("hex")).toBe("03fd3b41a8312dfe558f9e48927ba7f2bca55fbfca7f5dae4145bc7a26fed2d5");
          done();
        });
    });
    it("it should allow overwriting of the profile image", (done) => {
      performSignedMultipartFormRequest(server, "post", `/profile/${publicKey}`, "image", testImagePath).then(({ res }) => {
        res.should.have.status(200);
        done();
      });
    });
    ...
    it("it should delete the image", (done) => {
      performSignedJSONRequest(server, "post", `/profile/${publicKey}`, { image: null }).then(({ res }) => {
        res.should.have.status(200);
        done();
      });
    });
  ...
  describe("Cover Image API", () => {
    ...
    it("it should allow overwriting of the cover image", (done) => {
      performSignedMultipartFormRequest(server, "post", `/profile/${publicKey}`, "coverImage", testImagePath).then(({ res }) => {
        res.should.have.status(200);
        done();
      });
    });
    ...
    it("it should delete the cover image", (done) => {
      performSignedJSONRequest(server, "post", `/profile/${publicKey}`, { coverImage: null }).then(({ res }) => {
        res.should.have.status(200);
        done();
      });
    });
  ...
  describe("Internals", () => {
    it("it should verify the chain names on message", (done) => {
      profileLogic.init();
      const updateMock = jest.spyOn(profileLogic, "verifyPreferredChainNames").mockClear().mockImplementation(async () => {});
