...
describe("(Re)Tips", () => {
  ...
  describe("API", () => {
    ...
    it("it should sorting by latest", async () => {
      ...
      res.body.should.have.length(6);
      const ids = res.body.map(({ id }) => id);
      ids.should.eql(["2_v3", "1_v3", "2_v2", "1_v2", "2_v1", "1_v1"]);
    });
    it("it should sorting by score", async () => {
      ...
      ids.should.eql(["1_v2", "1_v1", "2_v1", "2_v3", "1_v3", "2_v2"]);
    });
    ...
    it("it should resolve new re-tips when they are added for v2", async () => {
      ...
      const [res] = await Promise.all([chai.request(server).get("/tips/await/retip/10_v2"), seedDB(fakeData)]);
      res.should.have.status(200);
    });
  });
  describe("DB", () => {
