...
export const FILTER_BLACKLIST = { [Op.notIn]: sequelize.literal('(SELECT "BlacklistEntries"."tipId" FROM "BlacklistEntries" WHERE "BlacklistEntries"."tipId" = "Tip"."id")') };
export const FILTER_TOKEN = (token) => ({ [Op.in]: sequelize.literal(`(SELECT id FROM TipsAggregation WHERE ("totalTokenAmount" @> ${sequelize.escape(`[{"token": "${token}"}]`)}))`) });
