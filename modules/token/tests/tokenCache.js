...
describe("Token Cache", () => {
  ...
  describe("aex9Token", () => {
    ...
    it("it should GET token info", async () => {
      ...
      sandbox.stub(CacheLogic, "getTokenRegistryState").callsFake(async () => [[contractAddress]]);
    ...
    it("it should ADD a token to be indexed", async () => {
      ...
      await cache.del(["getTokenMetaInfo", contractAddress]);
      const res = await chai.request(server).post("/tokenCache/addToken").send({ address: contractAddress });
      res.should.have.status(200);
    ...
    it("it should GET token balances for address", async () => {
      ...
      sandbox.stub(aeternity, "fetchTokenAccountBalances").callsFake(() => [[publicKey, 1000]]);
  ...
  describe("wordbazaar", () => {
    ...
    it("it should get a vote details from a word contract", async function () {
      ...
      res.body[0].should.be.deep.equal({
        ...
        subject: {
          VotePayout: ["ak_y87WkN4C4QevzjTuEYHg6XLqiWx3rjfYDFLBmZiqiro5mkRag"],
        },
        timeouted: true,
